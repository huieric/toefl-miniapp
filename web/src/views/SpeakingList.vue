<template>
  <div class="page-container">
    ...
    <div class="card" v-loading="loading">
      <el-empty v-if="!loading && !list.length" description="暂无口语题目" />
...
const goDetail = (row) => router.push(`/speaking/${row._id || row.id}`)

onMounted(async () => {
  loading.value = true
  try {
    const res = await questionAPI.getBySubject('speaking')
