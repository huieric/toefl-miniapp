'use strict';
// Test runner for FSRS, SM2, and PDF parser internals
// Run with: node test/run-tests.cjs

const { review, mapQualityToRating, retrievability, nextInterval } = require('../src/services/fsrs.js');
const { updateSM2, needsSameDayReview, sortByReviewPriority } = require('../src/services/sm2.js');
const { normalizeAnswer, parseAnswerKeyBlock, guessQuestionType, parseQuestions } = require('../src/services/pdf-parser.js')._internals;
...
test('mapQualityToRating: 0->1, 4->3, 5->4', () => {
...
// ---- SM2 ----
test('SM2 quality>=3: 递增 repetitions', () => {
  const r1 = updateSM2(2.5, 0, 0, 4);
  if (r1.repetitions !== 1) throw new Error('reps=1');
  const r2 = updateSM2(r1.easiness, r1.interval, r1.repetitions, 4);
  if (r2.repetitions !== 2) throw new Error('reps=2');
});
...
test('needsSameDayReview: quality<3=true', () => {
  if (!needsSameDayReview(2)) throw new Error('quality<3 should need review');
  if (needsSameDayReview(4)) throw new Error('quality>=3 should not');
});
