...
The consequences of insufficient sleep extend far beyond mere fatigue. Chronic sleep deprivation has been linked to a wide range of health problems, including cardiovascular disease, obesity, diabetes, and impaired immune function. Shift workers, who frequently experience disrupted sleep patterns, show higher rates of various health conditions, providing compelling evidence for sleep''s role in maintaining physical health.', 'approved')
ON CONFLICT (title, subject) DO UPDATE SET content = EXCLUDED.content, options = EXCLUDED.options, answer = EXCLUDED.answer, analysis = EXCLUDED.analysis, passage_text = EXCLUDED.passage_text;
